...
; Function to add an executable to Windows Startup
AddToStartup(executablePath)
{
    ...
    SplitPath, executablePath, exeName

    ; Define the path for the shortcut in the Startup folder
    shortcutPath := startupFolder . "\" . exeName . ".lnk"

    ; Create the shortcut
    FileCreateShortcut, %executablePath%, %shortcutPath%
    
    ; Check if the shortcut was created successfully
    if FileExist(shortcutPath)
    {
        ;MsgBox, 64, Success, Shortcut created in Startup folder:`n`n%shortcutPath%
        return true
    }
    else
    {
        MsgBox, 16, Error, Failed to create shortcut in Startup folder.
        return false
    }
}

; Example usage
executablePath=%A_ScriptDir%\TsukiSyncGUI.exe
iconPath=%A_ScriptDir%\sync.ico

CreateDesktopShortcut(executablePath, iconPath, "Tsuki Sync")

; Example usage
executablePath=%A_ScriptDir%\rcloneSync.exe
AddToStartup(executablePath)